...
Simulates a multi-turn conversation with the triage agent for Ana Costa
(low-complexity: healthy young adult, resolved tonsillitis, no active
conditions, no medications, no allergies) and checks:
...
async def run_test():
    ...
    with open(
        os.path.join(results_dir, "test_dialogue_ana_costa_results.json"), "w"
    ) as f:
        json.dump(
            {
                "patient": "Ana Costa",
