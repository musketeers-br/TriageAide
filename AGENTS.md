...
- **Ports**: 32782→1972 (IRIS SuperServer), 32783→52773 (Web/REST), 32784→53773, 8000→8000 (FHIR MCP), 8001→8001 (Triage MCP), 8002→8002 (Clinical Reasoning MCP), 7860→7860 (Gradio UI)
...
- `python/triage/.env` — Config (FHIR_BASE_URL, OPENAI_API_KEY) — NOT tracked in git
...
- `start_mcp_servers.sh` — Auto-start script (runs via custom-entrypoint.sh on container boot)
- `custom-entrypoint.sh` — Docker entrypoint wrapper that starts MCP servers before IRIS

### Running

MCP servers and Gradio start automatically with `docker compose up -d` via `custom-entrypoint.sh` → `start_mcp_servers.sh`. Seed data is loaded automatically on first boot.
...
3. **Container-internal vs host URLs**: Inside the container, use `http://localhost:52773/fhir/r4`. From the host, use `http://localhost:32783/fhir/r4`.
...
start_mcp_servers.sh            # Auto-start MCP servers (runs on container boot)
custom-entrypoint.sh            # Docker entrypoint wrapper
...
Dockerfile                      # Multi-stage Docker build (includes triage pip installs)
...
- Pip installs done manually inside a running container are lost on restart — add new dependencies to `Dockerfile` and `requirements.txt`
- MCP servers auto-start on container boot via `custom-entrypoint.sh` — check `/tmp/mcp_startup.log` for startup logs
